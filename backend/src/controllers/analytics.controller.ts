...
import { chartAnalyticsServive, summaryAnalyticsService } from "../services/analytics.service";
...
export const chartAnalyticsContoller = asyncHandler(
    async (req: Request, res: Response) => {
        const userId = req.user?._id;
        const { preset, from, to } = req.query;
        ...
        const chartData = await chartAnalyticsServive(userId, filter.dateRangePreset, filter.customFrom, filter.customTo);
